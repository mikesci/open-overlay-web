...
export default class ExternalActionHandler {
  ...
  HandleDataTransfer = (dataTransfer, dispatcher) => {

    if (dataTransfer.files.length > 0)
      this._handleUpload(dataTransfer.files, dispatcher);

    if (dataTransfer.items.length > 0)
      this._handleItems(dataTransfer.items, dispatcher);
  }

  _handleUpload = (files, dispatcher) => {
    if (!this._onUpload) {
      dispatcher.Dispatch("SHOW_TOAST", { message: "Uploading files is not enabled.  Sorry." })
      return;
    }
    ...
    let uploadJobs = [...files].map(file => {
      ...
      if (!fileTypeHandler) {
        dispatcher.Dispatch("SHOW_TOAST", { message: "Unrecognized file type!" });
        return null;
      }

      // create the toast we'll be uploading
      let toastKey;
      dispatcher.Dispatch("SHOW_TOAST", this.renderUploadProgress(0), null, responseToastKey => {
        toastKey = responseToastKey;
      });
    ...
    for(let uploadJob of uploadJobs) {
      if (uploadJob) {
        p = p.then(() => this._onUpload(uploadJob.file, uploadedBytes => {
          if (uploadJob.cancel) { return true; }
          dispatcher.Dispatch("SHOW_TOAST", this.renderUploadProgress(uploadJob, uploadedBytes / uploadJob.file.size), uploadJob.toastKey);
          return false;
        }))
        ...
        item.getAsString(val => {
          ...
          if (val.startsWith("{")) {
            ...
            try {
              ...
              dispatcher.Dispatch("SHOW_TOAST", { message: "Failed to parse clipboard data: invalid format."});
              return;
            }
            ...
            switch(obj.objectType) {
              case "layers":
                ...
                break;
              default:
                dispatcher.Dispatch("SHOW_TOAST", { message: "Failed to parse clipboard data: unknown object type." });
                break;
            }
  ...
  handleFailure = (uploadJob) => {
    uploadJob.dispatcher.Dispatch("SHOW_TOAST", { message: `Upload for ${uploadJob.file.name} failed!`, timeout: 2000 }, uploadJob.toastKey);
  }
